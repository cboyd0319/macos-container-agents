    ...
    fn var(&self, name: &str) -> Option<String> {
        match std::env::var(name) {
            ...
            Err(std::env::VarError::NotUnicode(_)) => {
                // Non-UTF-8 env values are treated as unset. (Codex logged this
                // via tracing::warn!; that coupling is removed in RunHaven.)
                None
            }
        }
    }
...
// Codex declared `#[path = "terminal_tests.rs"] mod tests;` here. RunHaven did
// not vendor that test file, so the declaration is omitted.
