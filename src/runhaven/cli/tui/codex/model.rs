...
impl Pet {
    ...
    /// first use.
    pub(crate) fn load_with_codex_home(value: &str, codex_home: Option<&Path>) -> Result<Self> {
        if path_like(value) {
    ...
    pub(crate) fn frame_cache_key(&self) -> Result<String> {
        let bytes = fs::read(&self.spritesheet_path)
            .with_context(|| format!("read {}", self.spritesheet_path.display()))?;
        let digest = Sha256::digest(&bytes);
        // Codex formatted the digest with `{digest:x}` against sha2 0.10's
        // `GenericArray` (which implements `LowerHex`). RunHaven pins sha2 0.11,
        // whose `Output` is a `hybrid_array::Array` without `LowerHex`, so the
        // same lowercase-hex string is built explicitly.
...
mod tests {
    ...
    // Codex's `load_builtin_pet_uses_app_catalog_storage` test was removed here:
    // it depended on `super::super::asset_pack::write_test_pack`, which RunHaven
    // does not vendor.
