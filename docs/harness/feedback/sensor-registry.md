...
| `scripts/apple_container_smoke.sh` | Repo-owned opt-in local smoke script | Prove Apple `container` runtime wiring, read-only workspace behavior, active-run status/logs-follow/stop cleanup, provider plan guidance, optional provider allowlist behavior with `--with-provider`, and exact temporary resource cleanup | Maintainers | Replace if the CLI gains an equivalent packaged smoke command or runtime architecture changes | Pre-Tauri checks, release prep, Apple `container` runtime changes, provider network changes, or run-observability changes |
