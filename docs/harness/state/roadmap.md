...
| Tauri/Svelte first scaffold | accepted | RunHaven has a minimal desktop app shell that reads setup/dashboard state without widening the CLI security boundary | `ui/`, `src-tauri/`, exact pins, lockfiles, Tauri capabilities, typed read-only commands, frontend tests | Start only from `docs/TAURI_UI_RESEARCH_PLAN.md` and `docs/TAURI_UI_GUARDRAILS.md`; no mutating commands in the first scaffold | Maintainers | Pending | Scaffold opens locally, read-only setup/dashboard commands work, exact pins pass, capability files are narrow, and focused Rust/frontend checks pass |
