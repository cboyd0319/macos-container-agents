...
| Run observability | Python code checks plus focused `tests/test_cli.py` run-record tests, git and non-git workspace cases, active-run listing/status/attach/logs-follow/stop checks, and manual `runhaven runs list/show/log/diff/active/status/attach/logs-follow/stop` checks |
